...
struct sophgo_sdhci_plat {
	struct mmc_config cfg;
	struct mmc mmc;
};

struct sophgo_sdhci_host {
...
static int sophgo_sdhci_bind(struct udevice *dev)
{
	struct sophgo_sdhci_plat *plat = dev_get_plat(dev);

	debug("%s\n", __func__);
	return sdhci_bind(dev, &plat->mmc, &plat->cfg);
}

static int sophgo_sdhci_phy_init(struct sophgo_sdhci_host *sophgo_host)
{
	struct sdhci_host *host = &sophgo_host->host;
...
static void sophgo_mmc_set_tap(struct sdhci_host *host, u16 tap)
{
...
int sophgo_mmc_execute_tuning(struct mmc *mmc, u8 opcode)
{
	...
	struct sophgo_sdhci_host *sophgo_host = dev_get_priv(mmc->dev);
	struct sdhci_host *host = &sophgo_host->host;
	u32 reg;
	...
	sophgo_mmc_set_tap(host, tuntap);
	debug("listsel:%d tuntap:%d\n", listsel, tuntap);
	return mmc_send_tuning(host->mmc, opcode, NULL);
}
#endif

int sophgo_get_cd(struct sdhci_host *host)
{
	u32 reg;
...
static const struct sdhci_ops sophgo_sdhci_ops = {
	.get_cd	= sophgo_get_cd,
};

#ifdef MMC_SUPPORTS_TUNING
static const struct sdhci_ops sophgo_sdhci_emmc_ops = {
	.get_cd	= sophgo_get_cd,
	.platform_execute_tuning = sophgo_mmc_execute_tuning,
};
#endif

static int sophgo_sdhci_probe(struct udevice *dev)
{
	struct mmc_uclass_priv *upriv = dev_get_uclass_priv(dev);
	struct sophgo_sdhci_plat *plat = dev_get_plat(dev);
	struct sophgo_sdhci_host *sophgo_host = dev_get_priv(dev);
	struct sdhci_host *host = &sophgo_host->host;
	...
	host->mmc->priv = host;
	upriv->mmc = &plat->mmc;
	sophgo_host->mmc_init_freq = fdtdec_get_uint(gd->fdt_blob, node, "mmc_init_freq", 200000);
	sophgo_host->mmc_trans_freq = fdtdec_get_uint(gd->fdt_blob, node, "mmc_trans_freq", 0);
	sophgo_host->is_64_addressing = fdtdec_get_bool(gd->fdt_blob, node, "64_addressing");
	sophgo_host->has_phy = fdtdec_get_bool(gd->fdt_blob, node, "has_phy");
	...
	ret = reset_get_by_name(dev, "sdhci", &sophgo_host->reset_ctl);
	if (ret) {
		debug("warning: reset_get_by_name failed\n");
	} else {
		// Try to solve 1.8 to 3.3v converter HW issue
		ret = reset_assert(&sophgo_host->reset_ctl);
		if (ret) {
			printf("%s failed assert reset\n", __func__);
			return ret;
		}

		ret = reset_deassert(&sophgo_host->reset_ctl);
		if (ret) {
	...
	ret = sdhci_setup_cfg(&plat->cfg, host, sophgo_host->mmc_trans_freq, sophgo_host->mmc_init_freq);

	if (ret)
		return ret;

	host->ops = &sophgo_sdhci_ops;
#ifdef MMC_SUPPORTS_TUNING
	if (host->index == EMMC_INDEX)
		host->ops = &sophgo_sdhci_emmc_ops;
#endif

	ret = sdhci_probe(dev);

	if (sophgo_host->has_phy)
		sophgo_sdhci_phy_init(sophgo_host);

	if (sophgo_host->is_64_addressing) {
...
static const struct udevice_id sophgo_sdhci_match[] = {
	{ .compatible = "sophgo,synopsys-sdhc" },
	{ /* sentinel */ }
};

U_BOOT_DRIVER(sophgo_sdhci) = {
	.name = "sophgo_sdhci",
	.id = UCLASS_MMC,
	.of_match = sophgo_sdhci_match,
	.bind = sophgo_sdhci_bind,
	.probe = sophgo_sdhci_probe,
	.priv_auto = sizeof(struct sophgo_sdhci_host),
	.plat_auto = sizeof(struct sophgo_sdhci_plat),
	.ops = &sdhci_ops,
};
